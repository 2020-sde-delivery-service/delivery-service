...
step1Handler.action(/Shipment \[(.+)\]/i, async (ctx) => {
    ...
    ctx.reply(strings.DCW_CONFIRM_MESSAGE + "\n\n" + strings.DCW_RECAP_MESSAGE(shipments[selected]),
...
step2Handler.action('accept', async (ctx) => {
    //send delivery
    await ctx.editMessageText(strings.DCW_CONFIRM_MESSAGE + "\n\n" + strings.DCW_RECAP_MESSAGE(shipments[selected]));
    ctx.reply(strings.DCW_ACCEPT_MESSAGE);
    return await ctx.scene.leave();
});
...
const deliveryConfirmWizard = new Scenes.WizardScene(
    'delivery-wizard',
    step1Handler,
    step2Handler
);
...
deliveryConfirmWizard.enter(async (ctx) => {
    shipments = await getShipments();
    message = strings.DCW_SHIPMENT_MESSAGE + "\n\n";
    let buttons = [];
    let line = [];
    let n = 0;
    let chr = String.fromCharCode(97 + n);
    shipments.forEach(e => {
        message += "Shipment [" + chr + "]\n" + strings.DCW_RECAP_MESSAGE(e) + "\n";
